...
<?php echo $group->name ?>
 ...
 <ul>
<!--   <li><a href="<?php echo site_url("groups/view/" . $group->ID) ?>"><?php echo lang("ctn_552") ?></a></li> -->
  <li><a href="<?php echo site_url("groups/view/" . $group->ID) ?>"><?php echo lang("ctn_990") ?></a></li>
  <li><a href="<?php echo site_url("groups/members/" . $group->ID) ?>"><?php echo lang("ctn_21") ?></a></li>
  <li class="active"><a href="<?php echo site_url("groups/albums/" . $group->ID) ?>"><?php echo lang("ctn_483") ?></a></li>
  <!-- <li><a href="<?php echo site_url("groups/events/" . $group->ID) ?>"><?php echo lang("ctn_553") ?></a></li> -->
 </ul>
...
<li class="album-image" style="margin: 5px; width: 200px; border-radius:5px;">


<?php if(isset($r->file_name)) : ?>
  <a class="thumbnail fancybox"  style="display: block;padding: 4px;margin-bottom: 5px;">
    <img src="<?php echo base_url() ?><?php echo $this->settings->info->upload_path_relative ?>/<?php echo $r->file_name ?>" width="140" alt="<?php echo $r->name . "<br>" . $r->description ?>" width="200" style="width: 100%; height: 160px; object-fit: cover;"></a>
    <?php else : ?>
     <a class="thumbnail fancybox" >
    <img src="<?php echo base_url() ?><?php echo $this->settings->info->upload_path_relative ?>/default_album.png" width="140" style="width: 100%; height: 160px; object-fit: cover;" alt="<?php echo $r->name . "<br>" . $r->description ?>"></a>
  <?php endif; ?>
   <?php if( (isset($member) && $member != null && $member->roleid == 1) || ($this->common->has_permissions(array("admin", "page_admin"), $this->user)) ) : ?> 
   <?php
  ...
  <div class="album-image-options">
    <!-- <a href="javascript:void(0)" onclick="edit_image(<?php echo $group->ID ?>)" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-cog"></span></a> -->
     <a href="<?php echo site_url("groups/delete_image/" . $r->ID . "/" . $this->security->get_csrf_hash()) ?>" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-trash"></span></a>
  </div>
<?php endif; ?>
<?php endif; ?>
...
<?php if( (isset($member) && $member != null && $member->roleid == 1) || ($this->common->has_permissions(array("admin", "page_admin"), $this->user)) ) : ?> 
<!--  <?php echo form_open_multipart(site_url("groups/adds_photo/" . $album->ID)) ?>
...
<?php echo form_close() ?>
 ...
 <?php echo form_open_multipart(site_url("groups/add_multi_photo/" . $album->ID)) ?>
...
{
	$.ajax({
		url: global_base_url + 'groups/edit_image/' + id,
		type: 'GET',
		data: {
		},
