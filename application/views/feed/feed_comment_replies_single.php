...
<div class="media">
  ...
  <div class="media-body">
   ...
   <p class="small-text"><a href="javascript: void(0)" class="<?php if($r->commentlikeid) : ?>active-comment-like<?php endif; ?> comment-like-link-<?php echo $r->ID ?>" onclick="like_comment(<?php echo $r->ID ?>)" id="comment-like-link-<?php echo $r->ID ?>"><?php echo lang("ctn_337") ?></a> <span class="comment-like-<?php echo $r->ID ?>" id="comment-like-<?php echo $r->ID ?>"><?php if($r->likes > 0) : ?>- <span class="glyphicon glyphicon-thumbs-up" id=""></span> <?php echo $r->likes ?><?php endif; ?></span> <?php if($r->userid == $this->user->info->ID || ($this->common->has_permissions(array("admin", "post_admin"), $this->user)) ) : ?>- [<a href="javascript: void(0)" onclick="delete_comment_reply(<?php echo $r->ID ?>)">X</a>]<?php endif; ?> - <?php echo $this->common->get_time_string_simple($this->common->convert_simple_time($r->timestamp)) ?></p>
  </div>
</div>
</div>
<?php endforeach; ?>