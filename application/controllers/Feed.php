...
//exit;
	...
	public function load_home_posts() 
	{
		...
		if($page == 0) {
			// Get promoted posts
			$promoted_posts = $this->feed_model->get_promoted_posts($this->user->info->ID,$this->user->info->gender);
		}
...
/************ Load Post in Popup ***************/
	...
	public function promote_post_pro($id) 
	{
		...
		if($this->user->info->points < $paisay) {
			$this->template->error(lang("error_161") . $paisay);
		}

		$this->user_model->update_user($this->user->info->ID, array(
			"points" => $this->user->info->points - $paisay
			)
		);
  ...
  {
  	$this->feed_model->update_status_feed($id,array(
  		'status_post' => 1,
  	));
  	echo 'true';
  }
