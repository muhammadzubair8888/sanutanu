...
class User_Model extends CI_Model 
{
	...
	public function mutual_frields_profile($userid)
	{
		$userid = addslashes($userid);
		$ID = $this->user->info->ID;
  ...
  //   		->limit(6)
  //   		->get("user_friends");
		$qry = "SELECT u.username, u.first_name, u.last_name, u.avatar,
    			u.online_timestamp, u.ID as userid,
    			user_friends.ID FROM `user_friends` inner join users as u on u.ID = user_friends.friendid WHERE friendid in( SELECT friendid from `user_friends` WHERE userid = '$ID' ) and userid = '$userid' ";
	...
	public function get_user_friends($userid, $limit) 
	{
		return $this->db
			->select("users.username, users.first_name, users.last_name,
				users.avatar,users.online_status, users.online_timestamp, users.ID as friendid")
