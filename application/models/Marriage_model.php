...
class Marriage_model extends CI_Model 
{
	...
	public function getquicksearch($one , $two , $three, $four, $five , $six, $seven)
	{	
	  $qry = "SELECT * FROM marriage_profile WHERE gender = '$one' AND age BETWEEN $three AND $four AND relegion = '$five' AND relegion_comunity = $six AND country = '$seven'";
		return $this->db->query($qry);
	}
	...
	public function get_new_profiles($page=0)
	{
		...
		$limit = 6;
		$limit2 = $page*$limit;
		return $this->db->where('users_id !=',$id)->order_by('ID', 'DESC')->limit($limit,$limit2)->get('marriage_profile');
	}
